...
static inline bool _send_async_event(lwip_event_packet_t ** e){
    return false; //_async_queue && xQueueSend(_async_queue, e, portMAX_DELAY) == pdPASS;
}

static inline bool _prepend_async_event(lwip_event_packet_t ** e){
    return false; //_async_queue && xQueueSendToFront(_async_queue, e, portMAX_DELAY) == pdPASS;
}

static inline bool _get_async_event(lwip_event_packet_t ** e){
    return false; //_async_queue && xQueueReceive(_async_queue, e, portMAX_DELAY) == pdPASS;
}
...
//         if((intptr_t)first_packet->arg == (intptr_t)arg){
//             free(first_packet);
//             first_packet = NULL;
...
//             return false;
//         }
//         if((intptr_t)packet->arg == (intptr_t)arg){
//             free(packet);
//             packet = NULL;
...
int8_t AsyncClient::_lwip_fin(tcp_pcb* pcb, int8_t err) {
    if(!_pcb || pcb != _pcb){
        // log_e("0x%08x != 0x%08x", (uint64_t)pcb, (uint64_t)_pcb);
        return ERR_OK;
    }
    // tcp_arg(_pcb, NULL);
    if(_pcb->state == LISTEN) {
